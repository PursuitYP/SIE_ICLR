...
def default_compute_score(data_source, solution_str, ground_truth, extra_info=None, sandbox_fusion_url=None, concurrent_semaphore=None, memory_limit_mb=None):
    ...
    elif data_source in ["webqsp-train", "webqsp-validation", "cwq-train"]:
        from . import search_r1_like_qa_em

        # res = search_r1_like_qa_em.compute_score(solution_str, ground_truth)
        res = search_r1_like_qa_em.compute_score_v2(solution_str, ground_truth)
    
    elif data_source in ["webqsp-test", "cwq-validation", "cwq-test", "grailqa-test"]:      # 数据集 test 不考虑 format_score
        from . import search_r1_like_qa_em

        # res = search_r1_like_qa_em.compute_score_v2(solution_str, ground_truth, format_score=0.0)
        res = search_r1_like_qa_em.compute_score(solution_str, ground_truth)

    elif data_source in ["kk-easy", "kk-hard"]:
        from . import kk
